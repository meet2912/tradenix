...
// --- HYBRID SEARCH (Instant Local + Live API) ---
...
const localAssets = [
    ...
    { symbol: "NIFTY 50", name: "Nifty 50", type: "Indices" },
...
if (symbolInput && searchDropdown) {
    symbolInput.addEventListener('input', function() {
        ...
        searchTimeout = setTimeout(async () => {
            try {
                const targetUrl = encodeURIComponent(`https://query2.finance.yahoo.com/v1/finance/search?q=${query}`);
                ...
                if (data.quotes && data.quotes.length > 0) {
                    const stockResults = data.quotes.filter(q => q.quoteType === "EQUITY" || q.quoteType === "ETF").slice(0, 6);
                    
                    if (stockResults.length > 0) hasResults = true;

                    stockResults.forEach(item => {
                        const symbol = item.symbol;
                        const name = item.shortname || item.longname || "Unknown Stock";
                        
                        let exchangeBadge = "";
                        if (symbol.endsWith(".NS")) exchangeBadge = `<span style="font-size: 0.7rem; background: #e0e7ff; color: #4f46e5; padding: 2px 4px; border-radius: 4px; margin-left: 5px;">NSE</span>`;
                        else if (symbol.endsWith(".BO")) exchangeBadge = `<span style="font-size: 0.7rem; background: #fef3c7; color: #f59e0b; padding: 2px 4px; border-radius: 4px; margin-left: 5px;">BSE</span>`;
                        ...
                        div.innerHTML = `
                            ...
                            <span class="market-type">Stock</span>
                        `;
                        
                        div.addEventListener('click', () => {
                            symbolInput.value = symbol.replace('.NS', '').replace('.BO', '');
